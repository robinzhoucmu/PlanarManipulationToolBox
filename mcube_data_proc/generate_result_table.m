        ...
        for ind_surface = 1:1:length(surface_types)
            for ind_shape = 1:1:length(shape_ids)
                for ind_ls_type = 1:1:length(ls_types)
                    ...
                    column_id = ind_shape;
                    pho = compute_shape_avgdist_to_center(shape_ids{ind_shape});
                    str_query = strcat('*', surface_types{ind_surface}, '*', shape_ids{ind_shape}, '*', ...
                        ls_types{ind_ls_type}, '*', num2str(vels), '*run', num2str(ind_run), '.mat');
                    files = dir(str_query);
                    load(files.name);
                    ...
                    disp_dev_percents = sqrt(sum(diff_trans.^2, 1)) ./ sqrt(sum((disp_gt_final - disp_gt_init).^2, 1));
